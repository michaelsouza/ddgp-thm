...
A branch mask $h \in \mathbb{F}_2^{B_P}$ may be produced by multiple generator combinations $\alpha \in A$ with $M\alpha = h$. Different choices of $\alpha$ may yield different violation vectors $V_F\alpha \in \mathbb{F}_2^{\mathcal{L}_F}$. To assign a well-defined labelled obstruction to $h$ — independent of which generator combination is used to produce it — we pass to a quotient of $\mathbb{F}_2^{\mathcal{L}_F}$.
...
Intuitively, $\omega_F(h)$ measures the net labelled obstruction of $h$, after discarding the purely presentational artifact $R_F$.
