...
The next lemma isolates the local geometric step needed below. It states that a mirror-compatible block is not merely an abstract bit mask: it is realized by a valid lateration reflection on the active subsystem.

\begin{lemma}[Block admissibility]
\label{lem:block-admissibility}
Let $C$ be a predecessor clique and let
\[
r(C)=\min\{q\in B_{\mathrm{c}}: U_q=C\}.
\]
Let $S\subseteq B_{\mathrm{c}}$ satisfy
\[
S\subseteq \{r(C),\ldots,n\}
\qquad\text{and}\qquad
S\cap C=\varnothing.
\]
Assume that $(S,C)$ is mirror-compatible with the active edge set $F$: for every active edge $e=\{a,b\}\in F$ with $|e\cap S|=1$, the endpoint fixed by the block belongs to $C$, i.e.,
\[
e\setminus S \subseteq C.
\]
For any nondegenerate branch embedding $x(s)$, let $s_{\mathrm{c}}=s|_{B_{\mathrm{c}}}$, let $R_C$ be the Euclidean reflection across $\operatorname{aff}\{x_c(s):c\in C\}$, and define
\[
y_i =
\begin{cases}
R_C(x_i(s)), & i\in S,\\
...
\end{cases}
\]
Then the coordinates $(y_i)_{i\in L}$ are obtained by the constrained branch code $s_{\mathrm{c}}\oplus \mathbf{1}_S$, and every active edge length is preserved:
\[
...
\begin{proof}
First consider the lateration edges. If $i\in S$, then $i\in B_{\mathrm{c}}$, hence $i\in L$. Since $L$ is closed under fixing sets, every predecessor $u\in U_i$ also lies in $L$, so the discretization edge $\{u,i\}$ belongs to $E_D[L]\subseteq F$. If $u\notin S$, then $\{u,i\}$ crosses the block boundary, and mirror-compatibility gives $u\in C$. Thus
\[
U_i\subseteq S\cup C.
\]
Consequently the transformed predecessor simplex of $i$ is the image of the original predecessor simplex under the same reflection $R_C$: predecessors in $S$ are reflected, and predecessors in $C$ are fixed by $R_C$.
...
The following lemma is the geometric direction encoded by the labeled violation matrix: zero net labeled violation gives a composition of mirror-compatible blocks, hence preserving every active edge.
...
\begin{proof}
Fix a full extension $s\in\mathbb{F}_2^B$ of $s_{\mathrm{c}}$; active-edge lengths are independent of free bits. We group the selected generators in $\alpha$ by mirror clique. For a fixed clique $C$, let $\alpha_C=\{g\in\mathcal{G}_C: \alpha_g=1\}$ be the selected generators with mirror clique $C$, and let $S_C = \bigoplus_{g \in \alpha_C} S_g$ be the XOR sum of their supports. The row $(e,C)$ of $V\alpha$ represents the net violation parity of generators with mirror $C$ against the active edge $e \in F$. Since $V\alpha = 0$, this parity is zero for every active edge $e \in F$ and every mirror clique $C$.

\paragraph{Part 1: Mirror-compatibility of blocks.}
We first show that the block $(S_C, C)$ is mirror-compatible with each active edge. Suppose that an active edge $e = \{a,b\} \in F$ crosses $S_C$, meaning that exactly one endpoint lies in $S_C$. Without loss of generality, assume $a \in S_C$ and $b \notin S_C$. Let $\chi_g: V \to \{0,1\}$ denote the indicator function of $S_g$ for each $g \in \alpha_C$. The indicator function of $S_C$ satisfies $\chi_{S_C}(v) \equiv \sum_{g \in \alpha_C} \chi_g(v) \pmod 2$. 
...
\paragraph{Part 2: Applying admissible blocks.}
By Part 1, every nonzero block $(S_C,C)$ is mirror-compatible with $F$. Moreover, $S_C\cap C=\varnothing$ because every generator support with mirror $C$ is disjoint from $C$.
