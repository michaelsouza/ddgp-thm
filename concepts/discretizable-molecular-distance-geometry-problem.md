...
Discretization is achieved by ordering the vertices of the graph such that each vertex (starting from the $K+1$-th) has known distances to its $K$ immediate predecessors.
...
2. **Discretization Order**: For every $i > K$, $v_i$ is adjacent to $v_{i-1}, \dots, v_{i-K}$. The edges $\{v_{i-j}, v_i\}$ for $j = 1, \dots, K$ are called **discretization edges** ($E_D$).
