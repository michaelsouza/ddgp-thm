...
In a discretizable distance geometry problem (DMDGP), every new vertex position $x_i$ (for $i > K$) is found by intersecting spheres centered at its $K$ predecessors, which gives two possible candidate positions. These two positions are reflections of each other across the hyperplane defined by the $K$ predecessors. In these notes, the first $K+1$ vertices are fixed, so the partial reflections considered for nontrivial solution counting start at $i=K+2$.
...
Let $x = (x_1, \dots, x_n)$ be a possible realization in $\mathbb{R}^K$ satisfying all discretization constraints. For each non-seed vertex $i > K+1$, let $R^i_x(y)$ represent the reflection of a point $y \in \mathbb{R}^K$ through the hyperplane defined by the positions of the predecessors $x_{i-K}, \dots, x_{i-1}$:
...
3. Any possible realization $x' \in \hat{X}$ can be generated from $x \in \hat{X}$ by a unique composition:
   $$x' = g_{K+2}^{s_1} \circ g_{K+3}^{s_2} \circ \dots \circ g_n^{s_{n-K-1}}(x)$$
   Where $s \in \{0, 1\}^{n-K-1}$ is a binary vector representing the nontrivial sequence of reflection choices after the initial rigid simplex has been fixed.
